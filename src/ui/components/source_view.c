...
static void source_view_setup_language(GtkSourceBuffer *buffer) {
...
static void source_view_setup_theme(GtkSourceBuffer *buffer) {
  g_return_if_fail(GTK_SOURCE_IS_BUFFER(buffer));
...
GtkWidget *source_view_new(GtkSourceBuffer **out_buffer) {
  ...
  source_view_setup_language(buffer);
  source_view_setup_theme(buffer);
